...
begin 
	...
	where r1.userId=@i and r2.userId != r1.userId and r1.movieId = r2.movieId and (select s from user_RatingNum where r2.userId=user_RatingNum.userId) > 4000
...
select id1, id2, relation from (select distinct userId from newratings where (select s from user_RatingNum where newratings.userId=user_RatingNum.userId) > 4000) as A cross apply TOP10(A.userId) as B
